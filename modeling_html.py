...
class HirachicalBert(FraBert):
    ...
    def forward(
        ...
        for layer in range(self.layer_num):
            ...
            # layer_node_output = layer_node_outputs[0]
                ...
                if len(word_logits) :
                    ...
                    token_labels = torch.cat((token_labels,layer_toten_labels),dim=0)
                else:
                    word_logits = self.cls(layer_text_output).view(-1,self.config.vocab_size)
                    token_labels = layer_toten_labels
            ...
            for i in range(batch_size):
                ...
                if x>0 and y == 0:
                    output_node = node_output[:x,:]
                    one_layer_output = output_node
                elif y>0 and x == 0:
                    output_text = text_output[:y,:]
                    one_layer_output = output_text
                else:
                    one_layer_output = torch.cat((output_node,output_text),dim = 0)
                    output_len = one_layer_output.size(1)
        ...
        text_mlm_loss = loss_fct(word_logits, token_labels.view(-1))
